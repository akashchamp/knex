...
class ColumnCompiler {
  ...
  getModifiers() {
    ...
    for (let i = 0, l = this.modifiers.length; i < l; i++) {
      ...
      // Most modifiers (e.g. 'nullable') do not make sense on increments
      // columns, but 'comment' and the MySQL/MariaDB column-positioning
      // modifiers 'first'/'after' are still valid and should not be
      // stripped -- see https://github.com/knex/knex/issues/4943
      if (
        !this.isIncrements ||
        ColumnCompiler._allowedIncrementsModifiers.includes(modifier)
      ) {
...
ColumnCompiler._allowedIncrementsModifiers = ['comment', 'first', 'after'];
